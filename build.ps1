...
task BuildScriptModule {
    ...
    if (Test-Path "$($BuildOptions.SourcePath)\ModuleHeader.txt") {
        Get-Content -Path "$($BuildOptions.SourcePath)\ModuleHeader.txt" | Add-Content -Path $modulePath
        Write-Verbose "Added ModuleHeader.txt to script module." 
    }
